...
summarizes configured S3 buckets, and checks RDS database status.
"""
...
# RDS instances to check - add DB identifiers as needed
DB_INSTANCES = ["terraform-web-db"]
...
def get_rds_status(db_identifier, region):
    """Return status and basic info for an RDS instance."""
...
def main():
    ...
    if DB_INSTANCES:
        ...
        for db_id in DB_INSTANCES:
            db_info = get_rds_status(db_id, "eu-west-2")

            print(f"  Database:   {db_id}")
            if db_info is not None:
                print(f"  Status:     {db_info['status']}")
                print(f"  Engine:     {db_info['engine']}")
                if db_info["publicly_accessible"]:
                    print(f"  Access:     WARNING - publicly accessible")
                else:
                    print(f"  Access:     Private (secure)")
            else:
